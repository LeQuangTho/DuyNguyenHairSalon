...
<body>
  <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
    ...
    @endif
    @if(in_array(Request::path(),['Store','collections','product-details','Shooping-Cart','search']))
      @if(Session::has('user'))
        @php
    ...
    <div class="cart-store">
      ...
      <div class="cart-hover">
        ...
        <div class="select-total">
          ...
          <h5 id="total-money">
            @if(Session::has('user'))
            {{number_format(getCartUser(Session::get('id_user'))['totalPrice'])}}
            @else
            @if(Session::has('Cart'))
    ...
    <div class="search-product">
        <form action="search" method="get">
            <input type="text" name="searchProduct" class="search-txt" placeholder="Tìm kiếm sản phẩm ở đây" required>
            <button type="submit" class="search-btn" value="search-pr"><i class="icon-search"></i></button>
        </form>
    </div>
    @endif
  </nav>
